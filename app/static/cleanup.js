(()=>{
  ...
  function updateSelected(){ $('selectedCount').textContent=selected.size; $('removeSelected').disabled=selected.size===0; }
  ...
  async function removeSelected(){
    ...
    const message=`Remove ${chosen.length} selected record(s) from Radarr/Sonarr?\n\nMedia files will NOT be deleted.${withMedia?`\n${withMedia} selected item(s) currently have media files; those files will stay on disk.`:''}`;
    if(!confirm(message))return;
    const button=$('removeSelected');button.disabled=true;button.textContent='Removing…';
    try{
      const payload={items:chosen.map((item)=>({instance_id:item.instanceId,item_id:item.itemId})),delete_files:false,add_import_exclusion:$('addExclusion').checked};
      ...
      notice(`Removed ${result.removedCount||0} record(s).${result.failedCount?` ${result.failedCount} failed.`:''}`,!result.failedCount);
    }catch(error){notice(error.message,false);}finally{button.textContent='Remove selected from app';updateSelected();}
  }
