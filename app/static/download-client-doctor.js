(()=>{
  ...
  function fixHint(p){
    const m=p.arrmedicMapping;
    if(!m||!m.hostPath||!m.containerPath)return '';
    const docker=`--mount type=bind,source=${m.hostPath},target=${m.containerPath}`;
    const compose=`- ${m.hostPath}:${m.containerPath}`;
    return `<details class="fix-hint"><summary>Suggested Docker mount</summary><p>Apply this manually to the affected container. ArrMedic will not recreate containers automatically.</p><small>docker run</small><code>${esc(docker)}</code><small>Compose volume</small><code>${esc(compose)}</code></details>`;
  }
  function render(data){
    ...
    $('results').innerHTML=items.map(item=>{
      ...
      const pathRows=(item.queuePaths||[]).slice(0,30).map(p=>`<tr><td>${esc(p.title)}</td><td>${esc(p.downloadClient||'—')}</td><td><code>${esc(p.reportedPath||'—')}</code></td><td><code>${esc(p.mappedPath||'—')}</code>${fixHint(p)}</td><td>${p.remoteMapping?'Mapped':'Direct'}</td></tr>`).join('')||'<tr><td colspan="5">No active queue paths to inspect.</td></tr>';
